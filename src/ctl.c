...
void print_help()
{
    ...
    printf("  start-split  - Start the timer/Split if timer is running\n");
    printf("  stop-reset   - Stop the timer/Reset the timer if its stopped\n");
    printf("  cancel       - Cancel the run\n");
    printf("  unsplit      - Unsplit the timer\n");
    printf("  skip         - Skip the current split\n");
...
bool sendToLibreSplit(const CTLCommand cmd)
{
    ...
    snprintf(socket_path, PATH_MAX, "%s/libresplit.sock", runtime_dir);
...
int main(int argc, char* argv[])
{
    ...
    if (strcmp(cmd, "start-split") == 0) {
        failed = sendToLibreSplit(CTL_CMD_START_SPLIT);
    } else if (strcmp(cmd, "stop-reset") == 0) {
        failed = sendToLibreSplit(CTL_CMD_STOP_RESET);
    } else if (strcmp(cmd, "cancel") == 0) {
        failed = sendToLibreSplit(CTL_CMD_CANCEL);
    } else if (strcmp(cmd, "unsplit") == 0) {
        failed = sendToLibreSplit(CTL_CMD_UNSPLIT);
    } else if (strcmp(cmd, "skip") == 0) {
        failed = sendToLibreSplit(CTL_CMD_SKIP);
    } else if (strcmp(cmd, "exit") == 0) {
        failed = sendToLibreSplit(CTL_CMD_EXIT);
    } else {
